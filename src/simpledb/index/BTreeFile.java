...
public class BTreeFile implements DbFile {
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		BTreePageId id = page.getRightSiblingId();
		BTreeLeafPage rightPage = id == null ? null : (BTreeLeafPage) getPage(tid,dirtypages,id,Permissions.READ_WRITE);
		if(rightPage != null){
			newPage.setRightSiblingId(id);
		...
		Field mid = tuples.next().getField(parent.keyField);
		BTreeEntry entry = new BTreeEntry(mid,page.getId(),newPage.getId());
		parent.insertEntry(entry);
		dirtypages.put(parent.getId(),parent);
		return field.compare(Op.LESS_THAN,mid) ? page : newPage;
	}
	...
	public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		BTreeEntry midEntry = entries.next();
		...
		return field.compare(Op.LESS_THAN, midEntry.getKey()) ? page : newPage;
	}
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int newPageNum = pageNum + siblingNum / 2;
		if(isRightSibling){
			Iterator<Tuple>iterator = sibling.iterator();
			while (iterator.hasNext() && pageNum < newPageNum){
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int leftSiblingNum = leftSibling.getNumEntries();
		int newPageNum = pageNum + leftSiblingNum / 2;
		Iterator<BTreeEntry>iterator = leftSibling.reverseIterator();

		BTreeEntry right = page.iterator().next();
		BTreeEntry left = iterator.next();
		...
		while (iterator.hasNext() && pageNum < newPageNum){
			entry = iterator.next();
			leftSibling.deleteKeyAndLeftChild(entry);
			page.insertEntry(entry);
			newPageNum++;
		}
	...
	public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int pageNum = page.getNumEntries();
		int leftSiblingNum = rightSibling.getNumEntries();
		int newPageNum = pageNum + leftSiblingNum / 2;
	...
	public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		if(id != null){
			BTreeLeafPage page = (BTreeLeafPage) getPage(tid,dirtypages,id,Permissions.READ_WRITE);
